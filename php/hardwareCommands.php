<?php
	//test file to create reservations
	error_reporting(E_ALL);
	ini_set('display_errors', 1);
	
	session_start();
	
	require "conf.php";
	require "HardwareCommandManager.php";
	require "ConnectDB.php";


		if(count($_POST) == 4
		&& !empty($_POST["dateStart"])
		&& !empty($_POST["dateEnd"])
		&& !empty($_POST["hardwareType"])
		&& !empty($_POST["quantity"])){
		


		$_POST["dateStart"] = trim($_POST["dateStart"]);
		$_POST["dateEnd"] = trim($_POST["dateEnd"]);
		//echo $_POST["hardwareType"];
		//launch bdd + tests bdd
		$db = new ConnectDB($dbConnection);

		$db = $db->connectToDB();
		$hardwareCommandManager = new HardwareCommandManager($db);

		//check if for room and site we got a room in the database, if not it doesn't exist
		/*if($hardwareCommandManager->checkRoom($_POST["room"], $_POST["site"]) == 0){
			$error = true;
			$listOfError[] = $listOfErrors[12];
		}*/

		//if a room exist, check if there isn't a reservated room between hours
		/*if(!is_null($_POST["idRoom"])){
			if($hardwareCommandManager->checkReservationRoom($_POST["idRoom"], $_POST["dateStart"], $_POST["dateEnd"]) == true){
				$error = true;
				$listOfError[] = $listOfErrors[13];
			}
		}*/

		//if there are errors , print them in the webpage
		/*if($error){
			$_SESSION["errors_form"] = $listOfError;
			$_SESSION["data_form"] = $_POST;
			echo json_encode($listOfError);
		
		//else create a reservation 
		}else{*/
			
			if($hardwareCommandManager->checkHardware($_POST["hardwareType"], $_POST["quantity"]) == 0){

				echo "AH";
			
				/*$error = true;
				$listOfError[] = $listOfErrors[12];*/
			}

			$hardwareCommand = new HardwareCommand([
						//'email' => $_POST["email"],
						'idHardware' => $_POST["idHardware"],
						'dateStart' => $_POST["dateStart"],
						'dateEnd' => $_POST["dateEnd"],
						'quantity'=> $_POST["quantity"],
						'hardwareType'=> $_POST["hardwareType"]
						]);
			

			$hardwareCommandManager->addHardwareCommand($hardwareCommand);
			

		

	}else{
		die("Erreur tentative, formulaire altérer ou erreur interne !");


	}
